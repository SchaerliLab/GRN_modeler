...
%% create the model (examples/node4_deeply_coupled_inducer.mat)
clean_up_GRN
Ecoli = Cell('Elowitz');
...
Ecoli = Ecoli.add_regulator('Repression_out','N3','HILL','P_N4','HILL','R1');
...
% prescribed inducer concentration
R1 = linspace(0,1000,100);

% store data: T1, err, T4, err, Amplitude in P_N1
data = zeros(length(R1),5);

%% simulate: the effect of 'R1'

tic
for i = 1:numel(R1)
    
    % change the promoter strength
    set(sbioselect(Mobj,'Name','R1'),'Value',R1(i));
...
%% plot the effect of 'R1' on the time period
...
hf = figure;
plot(R1,data(:,1),'LineWidth',2)
set(gca,'LineWidth',2,'Fontsize',16)
ylabel('\bf\boldmath$T$ / min','interpreter','latex','Fontsize',18)
xlabel('\bf\boldmath$\left[R_{1}\right]$ / $\mu$M','interpreter','latex','Fontsize',18)
exportgraphics(hf,'output/deeply_coupled_T_Elowitz_inducer.pdf','Resolution',300)

hf = figure;
plot(R1,data(:,5),'LineWidth',2)
set(gca,'LineWidth',2,'Fontsize',16)
ylabel('\bf\boldmath$A\left(\left[P_{N2}\right]\right)$ / molecule','interpreter','latex','Fontsize',18)
xlabel('\bf\boldmath$\left[R_{1}\right]$ /  $\mu$M','interpreter','latex','Fontsize',18)
exportgraphics(hf,'output/deeply_coupled_A_Elowitz_inducer.pdf','Resolution',300)
...
exportgraphics(hf,'output/deeply_coupled_Elowitz_inducer.pdf','Resolution',300)
